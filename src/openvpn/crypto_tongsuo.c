...
/**
 * @file Data Channel Cryptography Tongsuo-specific backend interface
 */
...
#warning Some Tongsuo EVP ciphers now support key lengths greater than MAX_CIPHER_KEY_LENGTH -- consider increasing MAX_CIPHER_KEY_LENGTH
#endif

#if MAX_HMAC_KEY_LENGTH < EVP_MAX_MD_SIZE
#warning Some Tongsuo HMAC message digests now support key lengths greater than MAX_HMAC_KEY_LENGTH -- consider increasing MAX_HMAC_KEY_LENGTH
#endif

#if HAVE_TONGSUO_ENGINE
...
setup_engine(const char *engine)
{
    ...
    if (engine)
    {
        ...
        {
            msg(M_INFO, "Initializing Tongsuo auto engine support");
        ...
        {
            crypto_msg(M_FATAL, "Tongsuo error: cannot load engine '%s'",
                       engine);
        }
        ...
        {
            crypto_msg(M_FATAL,
                       "Tongsuo error: ENGINE_set_default failed on engine '%s'",
                       engine);
        }

        msg(M_INFO, "Initializing Tongsuo support for engine '%s'",
            ENGINE_get_id(e));
    }
    return e;
}

#endif /* HAVE_TONGSUO_ENGINE */
...
#else  /* if HAVE_TONGSUO_ENGINE */
    msg(M_WARN, "Note: Tongsuo hardware crypto engine functionality is not available");
#endif
}
...
crypto_load_provider(const char *provider)
{
    ...
    msg(M_WARN, "Note: Tongsuo provider functionality is not available");
    return NULL;
#endif
}
...
crypto_print_openssl_errors(const unsigned int flags)
{
    ...
    while ((err = ERR_get_error()))
    {
        ...
        msg(flags, "Tongsuo: %s", ERR_error_string(err, NULL));
    }
}
...
show_available_ciphers(void)
{
    ...
    printf("See also tongsuo list -cipher-algorithms\n\n");
#endif
...
show_available_digests(void)
{
    ...
    printf("The following message digests are available for use with\n"
           ...
           "the --auth option.\n");
    printf("See also tongsuo list -digest-algorithms\n\n");
#endif
...
{
#if HAVE_TONGSUO_ENGINE /* Only defined for Tongsuo */
    ENGINE *e;

    printf("Tongsuo Crypto Engines\n\n");
    ...
    ENGINE_cleanup();
#else  /* if HAVE_TONGSUO_ENGINE */
    printf("Sorry, Tongsuo hardware crypto engine functionality is not available.\n");
#endif
}
...
cipher_valid_reason(const char *ciphername, const char **reason)
{
    ...
    {
        msg(D_LOW, "Cipher algorithm '%s' is known by Tongsuo library but "
