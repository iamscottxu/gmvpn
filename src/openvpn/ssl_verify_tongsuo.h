...
/**
 * @file Control Channel Verification Module Tongsuo backend
 */


#ifndef SSL_VERIFY_TONGSUO_H_
#define SSL_VERIFY_TONGSUO_H_
...
/** @name Function for authenticating a new connection from a remote Tongsuo peer
 *  @{ */

/**
 * Verify that the remote Tongsuo peer's certificate allows setting up a
 * VPN tunnel.
 * @ingroup control_tls
 *
 * This callback function is called every time a new TLS session is being
 * setup to determine whether the remote Tongsuo peer's certificate is
 ...
 * calls the Tongsuo library's \c SSL_CTX_set_verify() function with \c
 ...
 * Tongsuo-specific verification.
 *
 * @param preverify_ok - Whether the remote Tongsuo peer's certificate
 *                       past verification.  A value of 1 means it
 *                       verified successfully, 0 means it failed.
 * @param ctx          - The complete context used by the Tongsuo library
