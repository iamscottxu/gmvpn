...
#if defined(ENABLE_PKCS11) && defined(ENABLE_CRYPTO_TONGSUO)
                       ...
                       struct tls_root_ctx *const ctx)
{
    ...
    {
        msg(M_WARN, "PKCS#11: Failed to set cert and private key for Tongsuo");
        goto cleanup;
    }
    ret = 1;
...
#ifdef USE_NTLS
static int
xkey_load_from_pkcs11h_ntls_inner(pkcs11h_certificate_t certificate,
                                  struct tls_root_ctx* const ctx, bool sign_cert)
{
    int ret = 0;
    ...
    if (!x509)
    {
        msg(M_WARN, sign_cert ?
            "PKCS#11: Unable get x509 signature certificate object" :
            "PKCS#11: Unable get x509 encryption certificate object"
        );
        return 0;
    }
    ...
    {
        msg(M_WARN, sign_cert ?
            "PKCS#11: Failed to load signature private key into xkey provider" :
            "PKCS#11: Failed to load encryption private key into xkey provider"
        );
        goto cleanup;
    }
    /* provider took ownership of the pkcs11h certificate object -- do not free below */
    ...
    if (sign_cert)
    {
        ...
        {
            msg(M_WARN, "PKCS#11: Failed to set signature private key for tongsuo");
            goto cleanup;
        }

        if (!SSL_CTX_use_sign_certificate(ctx->ctx, x509))
        {
            msg(M_WARN, "PKCS#11: Failed to set signature cert for tongsuo");
            goto cleanup;
        }
    }
    else
    {
        ...
        {
            msg(M_WARN, "PKCS#11: Failed to set encryption private key for tongsuo");
            goto cleanup;
        }

        if (!SSL_CTX_use_enc_certificate(ctx->ctx, x509))
        {
            msg(M_WARN, "PKCS#11: Failed to set encryption cert for tongsuo");
            goto cleanup;
        }
    }
    ret = 1;
 ...
 * through xkey provider.
 *
 * @param sign_certificate     pkcs11h certificate object
 * @param enc_certificate      pkcs11h certificate object
 * @param ctx                  OpenVPN root tls context
 *
 * @returns                    1 on success, 0 on error to match
 *                             other xkey_load_.. routines
 */
static int
xkey_load_from_pkcs11h_ntls_inner(pkcs11h_certificate_t sign_certificate,
                                  ...
                                  struct tls_root_ctx* const ctx)
{
    ...
    int ret = 0;
    if ((ret = xkey_load_from_pkcs11h_ntls(sign_certificate, ctx, true)) != 0) {
        return ret;
    }
    if ((ret = xkey_load_from_pkcs11h_ntls(enc_certificate, ctx, false)) != 0) {
        return ret;
    }
    return ret;
}
                        ...
                        struct tls_root_ctx *const ssl_ctx)
{
    ...
    {
        msg(M_WARN, "PKCS#11: Cannot initialize tongsuo session");
        goto cleanup;
    }
    ...
    {
        msg(M_WARN, "PKCS#11: Cannot set private key for tongsuo");
        goto cleanup;
    }

    if (!SSL_CTX_use_certificate(ssl_ctx->ctx, x509))
    {
        msg(M_WARN, "PKCS#11: Cannot set certificate for tongsuo");
        goto cleanup;
    }
    ret = 0;
...
#ifdef USE_NTLS
int
pkcs11_init_tls_session_ntls_inner(pkcs11h_certificate_t certificate,
                                   struct tls_root_ctx* const ssl_ctx, bool sign_cert)
{
    int ret = 1;
    ...
    {
        msg(M_WARN, sign_cert ?
            "PKCS#11: Cannot initialize tongsuo session for signature certificate" :
            "PKCS#11: Cannot initialize tongsuo session for encryption certificate"
        );
        goto cleanup;
    }
    ...
    {
        msg(M_WARN, sign_cert ?
            "PKCS#11: Unable get evp object for signature certificate" :
            "PKCS#11: Unable get evp object for encryption certificate"
        );
        goto cleanup;
    }
    ...
    {
        msg(M_WARN, sign_cert ?
            "PKCS#11: Unable get signature certificate object" :
            "PKCS#11: Unable get encryption certificate object"
        );
        goto cleanup;
    }
    ...
    if (sign_cert) 
    {
        if (!SSL_CTX_use_sign_PrivateKey(ssl_ctx->ctx, evp))
        {
            msg(M_WARN, "PKCS#11: Cannot set signature private key for tongsuo");
            goto cleanup;
        }

        if (!SSL_CTX_use_sign_certificate(ssl_ctx->ctx, x509))
        {
            msg(M_WARN, "PKCS#11: Cannot set signature certificate for tongsuo");
            goto cleanup;
        }
    }
    else
    {
        if (!SSL_CTX_use_enc_PrivateKey(ssl_ctx->ctx, evp))
        {
            msg(M_WARN, "PKCS#11: Cannot set encryption private key for tongsuo");
            goto cleanup;
        }

        if (!SSL_CTX_use_enc_certificate(ssl_ctx->ctx, x509))
        {
            msg(M_WARN, "PKCS#11: Cannot set encryption certificate for tongsuo");
            goto cleanup;
        }
    }
    ret = 0;
...
int
pkcs11_init_tls_session_ntls(pkcs11h_certificate_t sign_certificate,
                             pkcs11h_certificate_t enc_certificate,
                             struct tls_root_ctx* const ssl_ctx)
{
    ASSERT(ssl_ctx);
...
#ifdef HAVE_XKEY_PROVIDER
    return (xkey_load_from_pkcs11h_ntls(sign_certificate, enc_certificate, ssl_ctx) == 0); /* inverts the return value */
#endif

    int ret = 0;
    if ((ret = pkcs11_init_tls_session_ntls_inner(sign_certificate, ssl_ctx, true)) != 0) {
        return ret;
    }
    if ((ret = pkcs11_init_tls_session_ntls_inner(enc_certificate, ssl_ctx, false)) != 0) {
        return ret;
    }
    return ret;
}
...
#endif /* defined(ENABLE_PKCS11) && defined(ENABLE_TONGSUO) */
