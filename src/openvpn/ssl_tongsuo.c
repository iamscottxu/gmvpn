...
/**
 * @file Control Channel Tongsuo Backend
 */
...
#ifdef USE_NTLS
    ctx->enable_ntls = false;
#endif /* ifdef USE_NTLS */
}

#ifdef USE_NTLS
void
tls_ctx_server_new_ntls(struct tls_root_ctx* ctx)
{
    ASSERT(NULL != ctx);

    ctx->ctx = SSL_CTX_new_ex(tls_libctx, NULL, NTLS_server_method());

    if (ctx->ctx == NULL)
    {
        crypto_msg(M_FATAL, "SSL_CTX_new NTLS_server_method");
    }
    if (ERR_peek_error() != 0)
    {
        crypto_msg(M_WARN, "Warning: TLS server context initialisation "
    ...
    SSL_CTX_enable_ntls(ctx->ctx);

    ctx->enable_ntls = true;
}
#endif /* ifdef USE_NTLS */
...
#ifdef USE_NTLS
void
tls_ctx_client_new_ntls(struct tls_root_ctx* ctx)
{
    ASSERT(NULL != ctx);

    ctx->ctx = SSL_CTX_new_ex(tls_libctx, NULL, NTLS_client_method());

    if (ctx->ctx == NULL)
    {
        crypto_msg(M_FATAL, "SSL_CTX_new NTLS_client_method");
    }
    if (ERR_peek_error() != 0)
    {
        crypto_msg(M_WARN, "Warning: TLS client context initialisation "
...
#ifdef USE_NTLS
void
convert_tls_list_to_openssl_ntls(char* openssl_ciphers, size_t len, const char* ciphers)
{
    /* Parse supplied cipher list and pass on to OpenSSL */
    size_t begin_of_cipher, end_of_cipher;
    ...
    /* Translate GB/T 38636 cipher suite names to OpenSSL names */
    begin_of_cipher = end_of_cipher = 0;
    for (; begin_of_cipher < strlen(ciphers); begin_of_cipher = end_of_cipher)
    {
        end_of_cipher += strcspn(&ciphers[begin_of_cipher], ":");
        cipher_pair = tls_get_cipher_name_pair_ntls(&ciphers[begin_of_cipher], end_of_cipher - begin_of_cipher);
        ...
        if (NULL == cipher_pair)
        {
            ...
            /* %.*s format specifier expects length of type int, so guarantee */
            /* that length is small enough and cast to int. */
            msg(D_LOW, "No valid translation found for NTLS cipher '%.*s'",
                constrain_int(current_cipher_len, 0, 256), current_cipher);
        }
        else
        {
            ...
            if (end_of_cipher - begin_of_cipher == current_cipher_len
                && 0 != memcmp(&ciphers[begin_of_cipher], cipher_pair->iana_name,
                    end_of_cipher - begin_of_cipher))
            {
                /* No GB/T 38636 name used, show warning */
                msg(M_WARN, "Deprecated NTLS cipher name '%s', please use GB/T 38636 name '%s'", cipher_pair->openssl_name, cipher_pair->iana_name);
            }
        }
        ...
        {
            msg(M_FATAL,
                "Failed to set restricted NTLS cipher list, too long (>%d).",
                (int)(len - 1));
        }
...
void
filter_ntls_ciphers_list_by_ssl_flags(char* filter_ciphers, size_t len,
    const char* ciphers, unsigned int ssl_flags)
{
    if (ciphers != NULL && strlen(ciphers) >= (len - 101))
    {
        msg(M_FATAL,
            "Failed to set restricted NTLS cipher list, too long (>%d).",
            (int)(len - 101));
    ...
    char* filter_ciphers_end = "";

    if (ssl_flags & SSLF_CLIENT_CERT_NOT_REQUIRED)
    {
        filter_ciphers_end = ":!kSM2DHE";
        msg(M_INFO, "");
    }
    else if (ssl_flags & SSLF_CLIENT_CERT_OPTIONAL)
    {
    ...
    {
        filter_ciphers_end = ":!kRSA:!kSM2";
        msg(M_INFO, "");
    }

    if (ciphers == NULL) strncpy(filter_ciphers, "DEFAULT", len);
    else strncpy(filter_ciphers, ciphers, len);
    strncpy(filter_ciphers + strlen(filter_ciphers), filter_ciphers_end, 
        strlen(filter_ciphers_end));
}

void check_ntls_ciphers_list_by_ssl_flags(struct tls_root_ctx* ctx, unsigned int ssl_flags)
{
    ...
    bool find_ecdhe = false, find_other = false;
    ...
    for (int i = 0; i < sk_SSL_CIPHER_num(sk); i++)
    {
        ...
        if (!find_ecdhe && (
            strncmp(cipher_name, "ECDHE-", 6) == 0 ||
            strncmp(cipher_name, "IBSDH-", 6) == 0 ||
            strncmp(cipher_name, "IBC-", 4) == 0))
            find_ecdhe = true;

        if (!find_other && (
            strncmp(cipher_name, "ECC-", 4) == 0 || 
            strncmp(cipher_name, "RSA-", 4) == 0))
            find_other = true;
    }

    if (ssl_flags & SSLF_CLIENT_CERT_NOT_REQUIRED)
    {
        if (!find_other) msg(M_FATAL, "The verify-client-cert is specified as none, "
            "but no ciphers found that allow ignoring client certificates. Please "
            "check the ntls-cipher parameter.");
    }
    else if (ssl_flags & SSLF_CLIENT_CERT_OPTIONAL)
    {
        if (!find_other) msg(M_WARN, "The verify-client-cert is specified as optional, "
            "but no ciphers found that allow ignoring client certificates. This "
            "will cause connections from clients that do not specify a client "
            "certificate to fail. Please check the ntls-ciphers parameter.");
        if (!find_ecdhe) msg(M_WARN, "The verify-client-cert is specified as optional, "
            "but no cipher found that supports client certificates. This "
            "will cause clients that specify a client certificate to be "
            "unable to connect using the client certificate. Please check "
            "the ntls-cipher parameter.");
    }
    else
    {
        if (!find_ecdhe) msg(M_FATAL, "The verify-client-cert is specified as require, "
            "but no cipher found that supports client certificates. Please "
            "check the ntls-cipher parameter.");
    }
}

void
tls_ctx_restrict_ciphers_ntls(struct tls_root_ctx* ctx, const char* ciphers,
                              unsigned int ssl_flags, bool filter_by_ssl_flags)
{
    ...
    char openssl_ciphers[4096];
    char filter_ciphers[4196];

    const char* tmp_ciphers = ciphers;

    if (ciphers != NULL) {
        convert_tls_list_to_openssl_ntls(openssl_ciphers, sizeof(openssl_ciphers), ciphers);
        tmp_ciphers = openssl_ciphers;
    }

    if (filter_by_ssl_flags && tmp_ciphers != NULL) {
        if (!SSL_CTX_set_cipher_list(ctx->ctx, tmp_ciphers))
        {
            crypto_msg(M_FATAL, "Failed to set restricted NTLS cipher list: %s",
                tmp_ciphers);
        }
        check_ntls_ciphers_list_by_ssl_flags(ctx, ssl_flags);
    }

    if (filter_by_ssl_flags) {
        filter_ntls_ciphers_list_by_ssl_flags(filter_ciphers, sizeof(filter_ciphers),
            tmp_ciphers, ssl_flags);
        tmp_ciphers = filter_ciphers;
    }
    else if (tmp_ciphers == NULL) {
        return;
    }
...
void
tls_ctx_check_cert_time_inner(const struct tls_root_ctx *ctx)
{
    int ret;
    const X509 *cert;
...
#ifdef USE_NTLS
void
tls_ctx_check_cert_time_ntls_inner(const struct tls_root_ctx* ctx, bool sign_cert)
{
    int ret;
    const X509* cert;

    if (sign_cert) cert = ctx->sign_cert;
    else cert = ctx->enc_cert;
    ...
    if (ret == 0)
    {
        if (sign_cert) msg(D_TLS_DEBUG_MED, "Failed to read signature certificate notBefore field.");
        else msg(D_TLS_DEBUG_MED, "Failed to read signature certificate notBefore field.");
    }
    if (ret > 0)
    {
        if (sign_cert) msg(M_WARN, "WARNING: Your signature certificate is not yet valid!");
        else msg(M_WARN, "WARNING: Your signature certificate is not yet valid!");
    }

    ret = X509_cmp_time(X509_get0_notAfter(cert), NULL);
    if (ret == 0)
    {
        if (sign_cert) msg(D_TLS_DEBUG_MED, "Failed to read signature certificate notAfter field.");
        else msg(D_TLS_DEBUG_MED, "Failed to read signature certificate notAfter field.");
    }
    if (ret < 0)
    {
        if (sign_cert) msg(M_WARN, "WARNING: Your signature certificate has expired!");
        else msg(M_WARN, "WARNING: Your signature certificate has expired!");
    }
}
#endif /* ifdef USE_NTLS */
void
...
#ifdef USE_NTLS
    if (ctx->enable_ntls) {
        ASSERT(NULL != ctx->sign_cert);
        ASSERT(NULL != ctx->enc_cert);
        tls_ctx_check_cert_time_ntls_inner(ctx, true);
        tls_ctx_check_cert_time_ntls_inner(ctx, false);
    }
    else
#endif /* ifdef USE_NTLS */
    tls_ctx_check_cert_time_inner(ctx);
}
...
tls_ctx_load_ecdh_params(struct tls_root_ctx *ctx, const char *curve_name)
{
    ...
    msg(D_LOW, "Your Tongsuo library was built without elliptic curve support."
...
#ifdef USE_NTLS
int
tls_ctx_load_pkcs12_ntls_inner(struct tls_root_ctx* ctx, const char* pkcs12_file,
                             bool pkcs12_file_inline, STACK_OF(X509)** ca, bool sign_pkcs12)
{
    FILE* fp;
    EVP_PKEY* pkey;
    X509* cert;
    ...
    if (!PKCS12_parse(p12, "", &pkey, &cert, ca))
    {
        ...
        *ca = NULL;
        if (!PKCS12_parse(p12, password, &pkey, &cert, ca))
        {
            ...
            if (management && (ERR_GET_REASON(ERR_peek_error()) == PKCS12_R_MAC_VERIFY_FAILURE))
            {
                management_auth_failure(management, sign_pkcs12 ? UP_TYPE_SIGN_PRIVATE_KEY : UP_TYPE_ENC_PRIVATE_KEY, NULL);
            }
#endif
            PKCS12_free(p12);
            return 1;
    ...
    if (sign_pkcs12)
    {
        if (!SSL_CTX_use_sign_certificate(ctx->ctx, cert))
        {
            crypto_msg(M_FATAL, "Cannot use signature certificate");
        }

        /* Load Private Key */
        if (!SSL_CTX_use_sign_PrivateKey(ctx->ctx, pkey))
        {
            crypto_msg(M_FATAL, "Cannot use signature private key");
        }
    }
    else {
        if (!SSL_CTX_use_enc_certificate(ctx->ctx, cert))
        {
            crypto_msg(M_FATAL, "Cannot use encryption certificate");
        }

        /* Load Private Key */
        if (!SSL_CTX_use_enc_PrivateKey(ctx->ctx, pkey))
        {
            crypto_msg(M_FATAL, "Cannot use encryption private key");
        }
    }
    ...
    if (!SSL_CTX_check_private_key(ctx->ctx))
    {
        if (sign_pkcs12) crypto_msg(M_FATAL, "Signature private key does not match the certificate");
        else crypto_msg(M_FATAL, "Encryption private key does not match the certificate");
    }
    return 0;
}

int
tls_ctx_load_pkcs12_ntls(struct tls_root_ctx* ctx, const char* sign_pkcs12_file,
                         ...
                         bool enc_pkcs12_file_inline, bool load_ca_file)
{
    ...
    ret = tls_ctx_load_pkcs12_ntls_inner(ctx, sign_pkcs12_file, sign_pkcs12_file_inline, &ca, true);
    if (0 != ret) return ret;
    ret = tls_ctx_load_pkcs12_ntls_inner(ctx, enc_pkcs12_file, enc_pkcs12_file_inline, &ca, false);
    if (0 != ret) return ret;
...
#ifdef ENABLE_CRYPTOAPI
void 
tls_ctx_load_cryptoapi_ntls(struct tls_root_ctx* ctx, const char* cryptoapi_sign_cert,
                            const char* cryptoapi_enc_cert)
{
    ...
    /* Load Certificate and Private Key */
    if (!SSL_CTX_use_CryptoAPI_certificate_ntls(ctx->ctx, cryptoapi_sign_cert, cryptoapi_enc_cert))
    {
        crypto_msg(M_FATAL, "Cannot load certificate \"%s\" or \"%s\" from Microsoft Certificate Store", cryptoapi_sign_cert, cryptoapi_enc_cert);
    }
}
#endif /* _WIN32 */
#endif /* ifdef USE_NTLS */
...
#ifdef USE_NTLS
void
tls_ctx_load_cert_file_ntls_inner(struct tls_root_ctx* ctx, const char* cert_file,
                            bool cert_file_inline, bool sign_cert)
{
    BIO* in = NULL;
    X509* x = NULL;
    int ret = 0;
    ...
    if (sign_cert)
        ret = SSL_CTX_use_sign_certificate(ctx->ctx, x);
    else
        ret = SSL_CTX_use_enc_certificate(ctx->ctx, x);
    if (ret)
    {
        tls_ctx_add_extra_certs(ctx, in, true);
    }
    ...
    if (!ret)
    {
        ...
        {
            if (sign_cert)
                crypto_msg(M_FATAL, "Cannot load inline signature certificate file");
            else
                crypto_msg(M_FATAL, "Cannot load inline encryption certificate file");
        }
        else
        {
            if (sign_cert)
                crypto_msg(M_FATAL, "Cannot load signature certificate file %s", cert_file);
            else
                crypto_msg(M_FATAL, "Cannot load encryption certificate file %s", cert_file);
        }
    }
    else
    {
...
void
tls_ctx_load_cert_file_ntls(struct tls_root_ctx* ctx, const char* sign_cert_file,
                            ...
                            bool enc_cert_file_inline)
{
    ...
    tls_ctx_load_cert_file_ntls_inner(ctx, sign_cert_file, sign_cert_file_inline, true);
    ctx->sign_cert = SSL_CTX_get0_certificate(ctx->ctx);
    tls_ctx_load_cert_file_ntls_inner(ctx, enc_cert_file, enc_cert_file_inline, false);
    ctx->enc_cert = SSL_CTX_get0_certificate(ctx->ctx);
}
#endif /* ifdef USE_NTLS */
...
#ifdef USE_NTLS
int
tls_ctx_load_priv_file_ntls_inner(struct tls_root_ctx* ctx, const char* priv_key_file,
                            bool priv_key_file_inline, bool sign_key)
{
    SSL_CTX* ssl_ctx = NULL;
    BIO* in = NULL;
    EVP_PKEY* pkey = NULL;
    ...
    if (sign_key) {
        if (!pkey || !SSL_CTX_use_sign_PrivateKey(ssl_ctx, pkey))
        {
#ifdef ENABLE_MANAGEMENT
            if (management && (ERR_GET_REASON(ERR_peek_error()) == EVP_R_BAD_DECRYPT))
            {
                management_auth_failure(management, UP_TYPE_SIGN_PRIVATE_KEY, NULL);
            }
#endif
            crypto_msg(M_WARN, "Cannot load signature private key file %s",
                print_key_filename(priv_key_file, priv_key_file_inline));
            goto end;
        }
    }
    else {
        if (!pkey || !SSL_CTX_use_enc_PrivateKey(ssl_ctx, pkey))
        {
#ifdef ENABLE_MANAGEMENT
            if (management && (ERR_GET_REASON(ERR_peek_error()) == EVP_R_BAD_DECRYPT))
            {
                management_auth_failure(management, UP_TYPE_ENC_PRIVATE_KEY, NULL);
            }
#endif
            crypto_msg(M_WARN, "Cannot load encryption private key file %s",
                print_key_filename(priv_key_file, priv_key_file_inline));
            goto end;
        }
    }
    ...
    if (!SSL_CTX_check_private_key(ctx->ctx))
    {
        if(sign_key) crypto_msg(M_FATAL, "Signature private key does not match the certificate");
        else crypto_msg(M_FATAL, "Encryption private key does not match the certificate");
    }
    ret = 0;
...
int
tls_ctx_load_priv_file_ntls(struct tls_root_ctx* ctx, const char* sign_priv_key_file,
                                  ...
                                  bool enc_priv_key_file_inline)
{
    ...
    int ret = 0;
    ret = tls_ctx_load_priv_file_ntls_inner(ctx, sign_priv_key_file, sign_priv_key_file_inline, true);
    if (0 != ret) return ret;
    ret = tls_ctx_load_priv_file_ntls_inner(ctx, enc_priv_key_file, enc_priv_key_file_inline, false);
    if (0 != ret) return ret;
    return 0;
}
#endif /* ifdef USE_NTLS */
...
#ifdef USE_NTLS
static int
tls_ctx_use_external_rsa_key(struct tls_root_ctx* ctx, EVP_PKEY* pkey, bool sign_key, bool enc_key)
#else
static int
...
#endif /* ifdef USE_NTLS */
{
    ...
    if (sign_key)
    {
        if (!SSL_CTX_use_sign_PrivateKey(ctx->ctx, privkey))
        {
            rsa = NULL; /* avoid double freeing it below */
            goto err;
        }
    }
    else if (enc_key)
    {
        if (!SSL_CTX_use_enc_PrivateKey(ctx->ctx, privkey))
        {
            rsa = NULL; /* avoid double freeing it below */
            goto err;
        }
    ...
    EVP_PKEY_free(privkey); /* this will down ref privkey and rsa */
    return 1;
...
#ifdef USE_NTLS
static int
tls_ctx_use_external_ec_key(struct tls_root_ctx* ctx, EVP_PKEY* pkey, bool sign_key, bool enc_key)
#else
static int
...
#endif /* ifdef USE_NTLS */
{
    ...
    if (EVP_PKEY_is_sm2(pkey)) {
        if (!EVP_PKEY_set_alias_type(pkey, EVP_PKEY_SM2)) {
            crypto_msg(M_NONFATAL | M_ERRNO, "failed to set sm2 alias type");
            goto err;
        }
    }
#endif
...
#ifdef USE_NTLS
int
tls_ctx_use_management_external_key_inner(struct tls_root_ctx *ctx, bool sign_key, bool enc_key)
#else
tls_ctx_use_management_external_key_inner(struct tls_root_ctx* ctx)
#endif /* ifdef USE_NTLS */
{
    int ret = 1;
    X509* cert;
...
#ifdef USE_NTLS
    if (sign_key) cert = ctx->sign_cert;
    else if (enc_key) cert = ctx->enc_cert;
    else
#endif /* ifdef USE_NTLS */
    cert = SSL_CTX_get0_certificate(ctx->ctx);
    ...
    {
#ifdef USE_NTLS
        if (!tls_ctx_use_external_rsa_key(ctx, pkey, sign_key, enc_key))
#else
    ...
    {
#ifdef USE_NTLS
        if (!tls_ctx_use_external_ec_key(ctx, pkey, sign_key, enc_key))
#else
    ...
    else if (EVP_PKEY_id(pkey) == EVP_PKEY_SM2)
    {
#ifdef USE_NTLS
        if (!tls_ctx_use_external_ec_key(ctx, pkey, sign_key, enc_key))
#else
    ...
    {
        crypto_msg(M_WARN, "management-external-key requires an RSA, EC or SM2 certificate");
        goto cleanup;
    }
#else  /* !defined(OPENSSL_NO_SM2) */
    else
    {
        crypto_msg(M_WARN, "management-external-key requires an RSA or EC certificate");
        goto cleanup;
    }
#endif /* !defined(OPENSSL_NO_SM2) */
#else  /* OPENSSL_VERSION_NUMBER > 1.1.0 dev && !defined(OPENSSL_NO_EC) */
#if !defined(OPENSSL_NO_SM2)
    else
    {
        crypto_msg(M_WARN, "management-external-key requires an RSA or SM2 certificate");
        goto cleanup;
    }
#else  /* !defined(OPENSSL_NO_SM2) */
    else
...
tls_ctx_use_management_external_key(struct tls_root_ctx* ctx)
{
    ...
    if (ctx->enable_ntls) {
        ASSERT(NULL != ctx->sign_cert);
        ASSERT(NULL != ctx->enc_cert);
        if (!tls_ctx_use_management_external_key_inner(ctx, true, false))
        {
            goto cleanup;
        }
        if (!tls_ctx_use_management_external_key_inner(ctx, false, true))
        {
            goto cleanup;
        }
    }
    else if (!tls_ctx_use_management_external_key_inner(ctx, false, false))
#else
    if (!tls_ctx_use_management_external_key_inner(ctx))
#endif /* ifdef USE_NTLS */
    {
        goto cleanup;
    }
                                ...
                                bool tls13)
{
    ...
    for (int i = 0; i < sk_SSL_CIPHER_num(sk); i++)
    {
        ...
        else if (NULL == pair)
        {
            ...
            printf("%s (No IANA name known to OpenVPN, use Tongsuo name.)\n",
                   cipher_name);
        }
        else
        {
                                     ...
                                     const char* tls_cert_profile)
{
    ...
    tls_ctx.ctx = SSL_CTX_new(NTLS_method());
    if (!tls_ctx.ctx)
    {
        crypto_msg(M_FATAL, "Cannot create SSL_CTX object");
    }

    SSL_CTX_set_max_proto_version(tls_ctx.ctx, NTLS_VERSION);
    tls_ctx_restrict_ciphers_ntls(&tls_ctx, cipher_list, 0, false);
    ...
    for (int i = 0; i < sk_SSL_CIPHER_num(sk); i++)
    {
        ...
        const tls_cipher_name_pair* pair =
            tls_get_cipher_name_pair_ntls(cipher_name, strlen(cipher_name));

        if (NULL == pair)
        {
            /* No translation found, print warning */
            printf("%s (No GB/T 38636 name known to OpenVPN, use Tongsuo name.)\n",
                cipher_name);
        }
        else
        {
...
show_available_curves(void)
{
    ...
    free(curves);
#else  /* ifndef OPENSSL_NO_EC */
    msg(M_WARN, "Your Tongsuo library was built without elliptic curve support. "
        "No curves available.");
...
{
    return OpenSSL_version(BABASSL_VERSION);
}
...
#endif /* defined(ENABLE_CRYPTO_TONGSUO) */
